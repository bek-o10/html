...
if (isset($_POST['submit'])) {
    ...
    if (in_array($_FILES['img']['type'], $allowed_types) && in_array($_FILES['img2']['type'], $allowed_types) && in_array($_FILES['img3']['type'], $allowed_types) && $_FILES['img']['size'] < 2000000 && $_FILES['img2']['size'] < 2000000 && $_FILES['img3']['size'] < 2000000) {
        ...
        if (move_uploaded_file($tempname, $folder_img) && move_uploaded_file($tempname2, $folder_img2) && move_uploaded_file($tempname3, $folder_img3)) {
            ...
            $stmt = mysqli_prepare($con, "INSERT INTO mobile_kel (title, text1, text2, text3, narx, narx2, readMore, img, img2, img3) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
...
if (isset($_POST['edit'])) {
    ...
    $stmt = mysqli_prepare($con, "UPDATE mobile_kel SET title = ?, text1 = ?, text2 = ?, text3 = ?, narx = ?, narx2 = ?, readMore = ? WHERE id = ?");
    mysqli_stmt_bind_param($stmt, "sssssssi", $title0, $title, $title2, $title3, $title4, $title5, $title6, $id);
    mysqli_stmt_execute($stmt);
    mysqli_stmt_close($stmt);
    if (!empty($_FILES['img']['name'])) {
        $stmt = mysqli_prepare($con, "UPDATE mobile_kel SET img = ? WHERE id = ?");
        mysqli_stmt_bind_param($stmt, "si", $img, $id);
        mysqli_stmt_execute($stmt);
        mysqli_stmt_close($stmt);
    }
    if (!empty($_FILES['img2']['name'])) {
        $stmt = mysqli_prepare($con, "UPDATE mobile_kel SET img2 = ? WHERE id = ?");
        mysqli_stmt_bind_param($stmt, "si", $img2, $id);
        mysqli_stmt_execute($stmt);
        mysqli_stmt_close($stmt);
    }
    if (!empty($_FILES['img3']['name'])) {
        $stmt = mysqli_prepare($con, "UPDATE mobile_kel SET img3 = ? WHERE id = ?");
...
if (isset($_GET['edit'])) {
    ...
    $result = mysqli_query($con, "SELECT * FROM mobile_kel WHERE id = $id");
...
$query = "SELECT * FROM mobile_kel LIMIT $offset, $records_per_page";
$res = mysqli_query($con, $query);
?>
